...
test('peça recebida por link aparece selecionada mesmo fora do primeiro lote', () => {
  const products = Array.from({length:55}, (_,i) => ({...fixtures[0],k:`V-${String(i).padStart(2,'0')}`}));
  const before = JSON.stringify(products);
  const harness = createControllerHarness({products});
  harness.controller.open('V-54');
  assert.equal(dressCards(harness)[0].dataset.code,'V-54');
  assert.equal(dressCards(harness)[0].getAttribute('aria-pressed'),'true');
...
// Legacy bridge cases moved to route-migration.test.cjs: attribution must survive.
